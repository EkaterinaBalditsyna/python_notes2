...
def create_note(self):
    ...
    self.note_text = tk.Text(self.note_window, height = 5, width = 25)
...
def view_notes(self):
    ...
    self.view_button = tk.Button(self.notes_window, text = "Посмотреть", command = self.view_note)
...
def view_note(self):
    ...
    self.note_lable.pack()

    self.note_text = tk.Text(self.edit_window, height = 10, width = 50)
    self.note_text.insert(tk.END, note_text)
    self.note_text.pack(padx = 10, pady = 10)

    self.save_button = tk.Button(self.edit_window, text = "Сохранить", command = self.edit_window.destroy)
    self.save_button.pack(side = tk.LEFT, padx = 10)
...
def delete_note(self):
    selected_note = self.notes_listbox.get(tk.ACNIVE)
